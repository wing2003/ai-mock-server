...
async def data_generator(model=None):
    """OpenAI 流式数据生成器，每0.5秒输出40个字符，使用 OpenAI Chat Completions 格式"""
    response_id = uuid.uuid4().hex
    sentence = "Hello! This is a test response from the OpenAI mock server to demonstrate streaming capabilities. Each chunk contains exactly 40 characters and is sent every 0.5 seconds. You can observe the real-time streaming behavior as the text appears gradually. This helps you understand how the API works in practice."
    ...
    try:
        yield f"data: {json.dumps(end_chunk.dict())}\n\n"
    except:
...
# for completion - OpenAI Chat Completions API
...
async def completion(request: Request, authorization: str = Header(None)):
    """OpenAI Chat Completions API - 统一使用 OpenAI 格式"""
    ...
    # Validate required fields for OpenAI format
    ...
    if data.get("model") in ["degraded", "slow_provider", "blocked"]:
        ...
        print(f"[DEGRADED MODE] Sleeping for {sleep_time:.1f} seconds")
        await asyncio.sleep(sleep_time)
    ...
    _model = _model if isinstance(_model, str) else "gpt-3.5-turbo"
        ...
        "choices": [
            {
                ...
                "message": {
                    "role": "assistant",
                    "content": "Hello! This is a test response from the OpenAI mock server. I am designed to demonstrate the API capabilities.",
                },
                "logprobs": None,
                "finish_reason": "stop",
            }
...
# for completion - OpenAI Completions API
@app.post("/completions")
@app.post("/v1/completions")
async def text_completion(request: Request):
    """OpenAI Completions API - 统一使用 OpenAI 格式"""
    data = await request.json()
    ...
    _model = data.get("model") or "gpt-3.5-turbo"
    _model = _normalize_model_for_response(_model if isinstance(_model, str) else "gpt-3.5-turbo")
            ...
            "choices": [
                {
                    ...
                    "index": 0,
                    "logprobs": None,
                    "text": "Hello! This is a test response from the OpenAI mock server. I am designed to demonstrate the API capabilities."
                }
            ],
            "created": int(time.time()),
            "model": _model,
...
async def data_generator_anthropic(model=None):
    """Anthropic 流式数据生成器，每0.5秒输出40个字符，使用 Anthropic Messages API 格式"""
    response_id = f"msg_{uuid.uuid4().hex}"
    sentence = "Hello! This is a test response from the Anthropic mock server to demonstrate streaming capabilities. Each chunk contains exactly 40 characters and is sent every 0.5 seconds. You can observe the real-time streaming behavior as the text appears gradually. This helps you understand how the API works in practice. Thank you for testing!"
    ...
    # 3. Send content_block_delta events - 每0.5秒输出40个字符
    chunk_size = 40
    for i in range(0, len(sentence), chunk_size):
        chunk_text = sentence[i:i + chunk_size]
        content_delta = {
    ...
    message_delta = {
        ...
        "usage": {"output_tokens": len(sentence.split())}
    }
...
# for completion - Anthropic Messages API
@app.post("/v1/messages")
async def completion_anthropic(request: Request):
    """Anthropic Messages API - 统一使用 Anthropic 格式"""
    data = await request.json()
    ...
    _model = data.get("model") or "claude-3-opus-20240229"
    ...
    if data.get("stream") == True:
        ...
        response = {
            ...
            "content": [
                {
                    "type": "text",
                    "text": "Hello! This is a test response from the Anthropic mock server. I am designed to demonstrate the API capabilities."
                }
            ],
            "model": _model,
            "stop_reason": "end_turn",
...
# for responses
async def responses_data_generator(input_text="", model=None):
    """Generator for streaming Responses API chunks, 每0.5秒返回4个字符"""
    ...
    sentence = f"Hello! This is a test response from the Responses API to demonstrate streaming capabilities. Each chunk contains exactly 4 characters and is sent every 0.5 seconds. You received input: '{input_text}'. The text appears gradually so you can observe real-time streaming behavior."
    ...
    # 3. Send text deltas - 每0.5秒返回4个字符
    chunk_size = 4
    for i in range(0, len(sentence), chunk_size):
        chunk_text = sentence[i:i + chunk_size]
        text_delta = {
