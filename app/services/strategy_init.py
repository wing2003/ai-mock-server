...
class StrategyInitService:
    ...
    async def init_default_strategies():
        ...
        strategies = [
            ...
            {"code": "api_key_balance_strategy", "name": "API Key 余额不足校验", "type": "key_health", "priority": 12, "params": {"min_balance": 0}, "handler": "app.strategies.key_health.ApiKeyBalanceStrategy"},
        ...
        async with AsyncSessionLocal() as db:
            for s in strategies:
                ...
                if not existing:
                    ...
                    # 将代码定义中的新增字段同步到已有记录（例如新增的 degrade_key），
                    # 仅追加缺失的 key，不覆盖已有值
                    current_params = dict(existing.default_params or {})
                    updated = False
                    for k, v in s["params"].items():
                        if k not in current_params:
                            ...
                            updated = True
                    if updated:
                        existing.default_params = current_params
                        logger.info(f"Updated default_params for strategy '{s['code']}': {current_params}")
            await db.commit()
